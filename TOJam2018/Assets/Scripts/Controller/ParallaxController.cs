...
namespace TOJAM
{
    public class ParallaxController : MonoBehaviour
    {
        /*
                ...
                private void HandleGameStateChanged(BoatRockerConstants.GameStates state, BoatRockerConstants.GameStates previousState)
                {
                    if (previousState == BoatRockerConstants.GameStates.Reset)
                        Reset();
                }

                private void Move(float cameraDelta)
                {
                    if (GameManager.Instance.State == BoatRockerConstants.GameStates.Ball)
                    {
                        //update lerp values
                        ...
                        this.transform.position = Vector3.Lerp(this.transform.position, new Vector3(target, this.transform.position.y, this.transform.position.z), Easing.Cubic.In(_lerpPositionValue));
                    }
                }
